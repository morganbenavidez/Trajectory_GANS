...
class STAR(torch.nn.Module):
    ...
    def mean_normalize_abs_input(self, node_abs, st_ed):
        ...
        for st, ed in st_ed:
            ...
            # Added .clone()
            node_abs[st:ed, :, 0] = (node_abs[st:ed, :, 0] - mean_x).clone()
            node_abs[st:ed, :, 1] = (node_abs[st:ed, :, 1] - mean_y).clone()
    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum == 0:
                ...
                with torch.no_grad():
                    # Added .clone()
                    temporal_input_embedded = self.dropout_in(self.relu(self.input_embedding_layer_temporal(nodes_current.detach())))
                    # Added .clone()
                    temporal_input_embedded[:framenum] = GM[:framenum, node_index].clone()
